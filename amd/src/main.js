define(
    ...
    return {
        debug: true,
        orgMap: function(uniqid, orgid) {
            var MAIN = this;
            if (MAIN.debug) console.log('local_webuntis/main:orgMap(uniqid, orgid)', uniqid, orgid);

            var card = $('#orgmap_' + uniqid + '_' + orgid);
            var trigger = $(card).find('i.fa');
